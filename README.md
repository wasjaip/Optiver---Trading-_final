...
Средняя Абсолютная Ошибка (MAE) рассчитывается по формуле:
...
- `y_i` — прогнозируемое значение для i-ой точки данных;
- `x_i` — наблюдаемое значение для i-ой точки данных.
